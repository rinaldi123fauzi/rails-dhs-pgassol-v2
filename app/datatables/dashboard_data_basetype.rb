class DashboardDataBasetype
  delegate :params, :link_to, :items_show_dashboard_index_path, :edit_item_path, :item_path, to: :@view
...
# link_to('Show', log_activity)
  ...
  def fetch_log_activities
    log_activities = Item.left_outer_joins(:brand,:disipline,:type,:product,:vendor).where(type_id: params[:type_id]).select("*, items.id AS iditems, vendors.name AS namevendors, disiplines.name AS namedisiplines").order("#{sort_column} #{sort_direction}, brands.name_brand")
